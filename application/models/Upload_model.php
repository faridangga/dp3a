...
class Upload_model extends CI_Model
{
    ...
    public function post_big_image_upload($file)
    {
        ...
        if ($this->my_upload->uploaded == true) {
            $this->my_upload->file_new_name_body = '["name"]';
    ...
    // thumbnail mobile
    // reccomended article mobile
    // related article
    public function post_slider_image_upload($file)
    {
        $this->my_upload->upload($file);
        if ($this->my_upload->uploaded == true) {
            $this->my_upload->file_new_name_body = 'image_600x460_' . url_title($file['name'], true);
            $this->my_upload->image_convert = 'jpg';
            $this->my_upload->jpeg_quality = 80;
            $this->my_upload->process('./uploads/images/');
            $image_path = "uploads/images/" . $this->my_upload->file_dst_name;
    ...
    public function post_mid_image_upload($file)
    {
        $this->my_upload->upload($file);
        if ($this->my_upload->uploaded == true) {
            $this->my_upload->file_new_name_body = 'image_380x226_' . uniqid();
            $this->my_upload->image_convert = 'jpg';
            $this->my_upload->jpeg_quality = 50;
